...
def _iter_alignment_dirs(ctx: Dict[str, Any]) -> List[Path]:
    """Return unique candidate alignment directories from legacy and run-specific paths."""
    candidates: List[Path] = list(ctx.get("ALIGN_DIRS", []))
    run_v1_dir = ctx.get("RUN_V1_DIR")
    if isinstance(run_v1_dir, Path):
        candidates.extend(
            [
                run_v1_dir / "aligned_train",
                run_v1_dir / "aligned_holdout",
                run_v1_dir / "aligned_raw_v1",
                run_v1_dir / "aligned_raw_v1_improved",
                run_v1_dir / "aligned_raw_v1_rerun",
            ]
        )
...
def _extract_suspicious_counts(report: Dict[str, Any]) -> Dict[str, float]:
    """Extract suspicious counters from either top-level or nested QA report fields."""
    ...
    if pd.isna(suspicious_count):
        suspicious_count = qa.get("suspicious_chunks", np.nan)

    dropped_suspicious_count = report.get("dropped_suspicious_chunks", np.nan)
    if pd.isna(dropped_suspicious_count):
        dropped_suspicious_count = qa.get("dropped_suspicious_chunks", np.nan)

    return {
        "suspicious_count": float(suspicious_count) if pd.notna(suspicious_count) else np.nan,
        "dropped_suspicious_count": float(dropped_suspicious_count) if pd.notna(dropped_suspicious_count) else np.nan,
    }


def _infer_chunk_count_without_manifest(align_dir: Path) -> float:
    """Fallback chunk count when manifest is unavailable."""
    audio_dir = align_dir / "audio"
    text_dir = align_dir / "text"
    if audio_dir.exists():
        return float(len(list_audio_files(audio_dir)))
    if text_dir.exists():
        return float(len(list(text_dir.glob("*.txt"))))
    return np.nan
...
def run_data_acquisition(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for align_dir in _iter_alignment_dirs(ctx):
        manifest_path = align_dir / "manifest.json"
        report_path = align_dir / "report.json"

        chunk_count = np.nan
        suspicious_count = np.nan
        dropped_suspicious_count = np.nan
        status = "missing"

        if manifest_path.exists():
            manifest = read_json(manifest_path)
            if isinstance(manifest, list):
                chunk_count = float(len(manifest))
            elif isinstance(manifest, dict):
                chunk_count = float(len(manifest.get("items", [])))
            status = "manifest"
        else:
            fallback_count = _infer_chunk_count_without_manifest(align_dir)
            if pd.notna(fallback_count):
                chunk_count = fallback_count
                status = "fallback_from_files"

        if report_path.exists():
            report = read_json(report_path)
            if isinstance(report, dict):
                suspicious_stats = _extract_suspicious_counts(report)
                suspicious_count = suspicious_stats["suspicious_count"]
                dropped_suspicious_count = suspicious_stats["dropped_suspicious_count"]
        ...
        alignment_stats.append(
            {
                ...
                "dropped_suspicious_count": dropped_suspicious_count,
                "status": status,
    ...
    if not alignment_df.empty:
        best_aligned = int(pd.to_numeric(alignment_df["chunk_count"], errors="coerce").max(skipna=True) or 0)
    else:
        best_aligned = 0
    ...
    display(alignment_df[display_cols] if set(display_cols).issubset(alignment_df.columns) else alignment_df)
