...
from src.utils.metrics import evaluate_predictions  # noqa: E402
...
def evaluate_model(
    ...
    for batch in loader:
        ...
        for index, text in enumerate(decoded):
            predictions_raw.append(text)
            references_raw.append(batch["text"][index])
    ...
    wer_score = float(metrics["wer"])
    cer_score = float(metrics["cer"])
